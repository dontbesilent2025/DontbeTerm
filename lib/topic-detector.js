...
async function detectTopicViaClaudeCode(bufferContent) {
  return new Promise((resolve) => {
    ...
    if (!cleaned || cleaned.length < 50) {
      console.log('[Topic Detector] Buffer too short, using default');
      resolve('新对话');
      return;
    }
    ...
    console.log('[Topic Detector] Executing command:', command);
    ...
    claudeProcess.stdout.on('data', (data) => {
      ...
      console.log('[Topic Detector] stdout:', data.toString());
    });

    claudeProcess.stderr.on('data', (data) => {
      errorOutput += data.toString();
      console.log('[Topic Detector] stderr:', data.toString());
    });
    ...
    claudeProcess.on('close', (code) => {
      ...
      if (code === 0 && output.trim()) {
        ...
        if (topic && topic !== '新对话' && topic.length > 0 && topic.length < 20) {
          ...
          console.log('[Topic Detector] Topic invalid, using default');
          resolve('新对话');
        }
      } else {
